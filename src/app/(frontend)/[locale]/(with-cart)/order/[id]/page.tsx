...
const OrdersPage = async ({
  ...
  const c = await getTranslations("CheckoutForm.");
            ...
            const productImage =
              product.variants && product.variantSlug
                ? ((product.variants.find(
                    (variant) => product.variantSlug === variant.variantSlug
                ...
                <div className="flex flex-auto flex-col">
                  <div>
                    ...
                    <p className="mt-2 text-sm text-gray-500">
                      {
                        product.colors?.find(
                          (color) => color.slug === selectedVariant?.color
                        )?.label
                      }
                      {selectedVariant?.color && selectedVariant?.size && ", "}
                      {
                        product.sizes?.find(
                          (size) => size.slug === selectedVariant?.size
                        )?.label
                      }
                    </p>
                    {product.description && (
                ...
                <dd className="mt-2 text-gray-700">
                  <address className="not-italic">
                    ...
                    <span className="block">
                      ...
                      {c(order.shippingAddress.country)}
                    </span>
            ...
            <dl className="space-y-6 border-t border-gray-200 pt-10 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <dd className="text-gray-900">
                  {formatPrice(
                    order.orderDetails.totalWithShipping,
                    order.orderDetails.currency,
                    locale
                  )}
                </dd>
